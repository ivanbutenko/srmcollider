...
struct SRMPrecursor
{
  ...
  void get_fragment_masses(std::vector<double>& result, double ch, const SRMParameters& params) const
  {
      ...
      start = 0;
      j = 0; 
      if(isotope_modification == NOISOTOPEMODIFICATION) {
      ...
      } else if(isotope_modification == N15_ISOTOPEMODIFICATION ) {
      ...
      {
        throw "Isotope modification nr that is not supported found.";
      }
      ...
      //
      if (params.yions) 
          // result.push_back( = acc_mass + 2*MASS_H + MASS_OH;
          for (int j=0; j<scounter-1; j++) result.push_back(acc_mass - tmp[j] + 2*MASS_H + MASS_OH);
      if (params.bions) for (int j=0; j<scounter-1; j++) result.push_back(tmp[j] + MASS_H );

      if (params.aions) for (int j=0; j<scounter-1; j++) result.push_back(tmp[j] + MASS_H - MASS_CO );
      if (params.cions) for (int j=0; j<scounter-1; j++) result.push_back(tmp[j] + MASS_H + MASS_NH3 );
      if (params.xions) for (int j=0; j<scounter-1; j++) result.push_back(acc_mass - tmp[j] + MASS_CO + MASS_OH);

      if (params.zions) 
          // result.push_back( = acc_mass + 2*MASS_H + MASS_OH - MASS_NH3);
          for (int j=0; j<scounter-1; j++) result.push_back(acc_mass - tmp[j] + 2*MASS_H + MASS_OH - MASS_NH3); 

      if (params.aMinusNH3) for (int j=0; j<scounter-1; j++) result.push_back(tmp[j] + MASS_H - MASS_CO - MASS_NH3); 
      if (params.bMinusH2O) for (int j=0; j<scounter-1; j++) result.push_back(tmp[j] + MASS_H - MASS_H2O);
      if (params.bMinusNH3) for (int j=0; j<scounter-1; j++) result.push_back(tmp[j] + MASS_H - MASS_NH3);
      if (params.bPlusH2O)  for (int j=0; j<scounter-1; j++) result.push_back(tmp[j] + MASS_H + MASS_H2O);

      if (params.yMinusH2O) {
          // result.push_back(acc_mass + MASS_H);
          for (int j=0; j<scounter-1; j++) 
              result.push_back(acc_mass - tmp[j] + MASS_H); }
      if (params.yMinusNH3) {
          //result.push_back(acc_mass + 2*MASS_H + MASS_OH - MASS_NH3);
          for (int j=0; j<scounter-1; j++) 
              result.push_back(acc_mass - tmp[j] + 2*MASS_H + MASS_OH - MASS_NH3);
      }
      ...
      if (params.MMinusH2O) result.push_back(acc_mass);
      if (params.MMinusNH3) result.push_back(acc_mass + MASS_H2O - MASS_NH3);

      // calculate the charged mass of the fragments
      for (size_t j=0; j<result.size(); j++) result[j] = (result[j] + (ch-1)*MASS_H)/ch;
      //return frg_cnt;
  }
